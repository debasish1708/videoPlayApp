...
@section('title', 'Banner Creation - Forms')
...
@section('page-script')
  ...
  <script>
    ...
    const form = document.getElementById('banner-create-form');
    if (form) {
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
    const createBannerBtn = document.querySelector('#submit-banner');
    const form = document.getElementById('banner-create-form');
    if (form) {
      form.addEventListener('submit', function (e) {
...
@section('content')
  <form id="banner-create-form" action="{{ route('banners.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="col-md-9">
      ...
      @enderror
        ...
        <div class="mt-auto">
        <button type="submit" id="submit-banner" class="btn btn-card-block-custom btn-primary">Submit</button>
        </div>
      </div>
      </div>
    </div>
    ...
    <div class="col-md-3">
      <div class="card h-100">
      <h5 class="card-header text-center">Upload Image <span class="text-danger">*</span></h5>
